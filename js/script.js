...
//Declarando uma lista de carros
    ...
    // const novaLista = carros.map( function(c){
    //     console.log(c)
    //     return c;
    // })
    ...
    // //Realizando uma filtragem no array de obj e retornando apenas os carros com a letra "M" no nome.
    // const carrosFiltrados = carrosObj.filter(c=> c.id % 2 == 0);

    // console.log(carrosFiltrados);

    //Utilizando find para encontrar um determinado carro:
    // const resultados = carrosObj.find(c=> c.id % 2 == 0);
    // console.log(resultados)

    //Mesclando dois arrays de obj, um de veículo outro de preços:
    const carros = [
        {"id":1, "nome": "Ferrari"},
        {"id":2, "nome": "Porsche"},
...
//console.log(carrosPrecidicados);

const carrosPrecificados = carros.map((c)=>{
    const preco = precos.find(p.id === c.id);
